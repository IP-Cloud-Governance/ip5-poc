...
from ip5_poc.services import project_service, oscal_service, azure_service
...
from ip5_poc.services.azure_service import (
    get_rg_pattern,
    get_subscription_id_from_path,
...
async def create_assessment(
    ...
    """
    Create an assessment result based on the tasks defined in the assessment plan.
    The result is added to an existing result set. Each exution adds another assessment to the lis
    """
    ...
    # Log of actions which have been beformed by the analyzing entity in exeuting an assessment plan ... anlyzing xyz
    assessment_log_entries: list[Entry1] = []

    # Controls meaning excerpts of the catalog (e.g si001) which have been identitfied / analyzes
    include_controls: list[OscalCompleteOscalAssessmentCommonSelectControlById] = []

    # Result of the assessment ... in this case whether a component is non-/compliant
    findings: list[OscalCompleteOscalAssessmentCommonFinding] = []

    # Excute tasks with deployment realated actions ... e.g. applyin policy definitions to scope
    await _execute_deploy_tasks(
        assessment_plan=assessment_plan,
        project=project,
        credential=credential,
        db=db,
        assessment_log_entries=assessment_log_entries,
    )

    # Excute analyzing tasks ... e.g. to fetch the current compliance state for the deployed policies
    reviewed_controls_id: list[str] = []
    await _execute_check_tasks(
        assessment_plan=assessment_plan,
        project=project,
        credential=credential,
        db=db,
        ...
        reviewed_controls_id=reviewed_controls_id,
        findings=findings,
    )

    # OSCAL model of an assessment execution performed for a given assessment plan
    assessment_result = OscalCompleteOscalArResult(
        ...
        assessment_log=AssessmentLog(entries=assessment_log_entries),
    ...
    if existing_assessment_result:
        logger.info(
            f"Assessment result already found for plan {assessment_plan.uuid.root}. Add result to existing result set"
        )
        updated_ap = await db[
            MongoDBCollections.ASSESSMENT_RESULTS.value
        ].find_one_and_update(
            ...
            update={
                "$push": {
                    "assessment-results.results": jsonable_encoder(
                        assessment_result.model_dump(by_alias=True, exclude_none=True)
                    )
                }
            },
            projection={"_id": 0},
        ...
        return Model.model_validate(updated_ap).model_dump(
            by_alias=True, exclude_none=True
        )
    else:
        logger.info(
            f"No assessment result there yet, creating new assessment result for assessment plan {assessment_plan.uuid.root}"
        )
        new_ap = Model6(
            assessment_results=OscalCompleteOscalArAssessmentResults(
                uuid=str(uuid.uuid4()),
                import_ap=OscalCompleteOscalArImportAp(href=assessment_plan.uuid.root),
        ...
        await db[MongoDBCollections.ASSESSMENT_RESULTS.value].insert_one(
            jsonable_encoder(new_ap)
        )
        return new_ap


async def _execute_check_tasks(
    ...
    """
    Execute the tasks in the assessment plan for the azure plattform -> marked witht CacTaskType.AZURE_CHECK_INITAITVE
    """
    ...
    for check_task in check_tasks:
        search_id = next(
            (
                ...
                if prop.name.root == OscalPropertyIdentifier.CAC_PROJECT_SEARCH_ID.value
            ),
            None,
        )
        control_id = next(
            (
                prop.value.root
                for prop in check_task.props
                if prop.name.root == OscalPropertyIdentifier.OSCAL_CONTROL_ID.value
            ),
            None,
        )
        component_id = next(
            (
                prop.value.root
                for prop in check_task.props
                if prop.name.root == OscalPropertyIdentifier.OSCAL_COMPONENT_ID.value
            ),
            None,
        )
        azure_resource_id = next(
            (
                prop.value.root
                for prop in check_task.props
                if prop.name.root == OscalPropertyIdentifier.AZURE_RESOURCE_ID.value
            ),
            None,
        )
        azure_policy_id = next(
            (
                prop.value.root
                for prop in check_task.props
                if prop.name.root == OscalPropertyIdentifier.AZURE_POLICY.value
            ),
            None,
        )
        if (
            ...
            or azure_resource_id is None
            or azure_policy_id is None
        ):
            logger.info(
        ...
        if azure_path is None:
            logger.info(
                f"No serach path with id {search_id} was found in project {project.id}. Check of control_id {control_id} is therfore skipped"
            )

        # Instantiate an azure policy client to peform actions on azure cloud plattform
        ...
        if policy_definition is None:
            logger.info(
                f"No policy definition was found with id {control_id} and search_id {search_id}. Has it already been assigned?"
            )
            continue
        ...
        if policy_definition.assignment is None:
            logger.info(
                f"No assignment has been found for the policy initiative {policy_definition.id}"
            )
            continue

        logger.info(f"Check policy compliance for ressource {component_id}")

        policy_compliance = azure_service.get_policy_results_for_project(
        ...
        non_compliant = any(
            r.compliance_state == "NonCompliant" for r in policy_compliance
        )
        ...
        if non_compliant:
            findings.append(
                _create_fiding(
                    state=OscalControlComplianceState.NON_COMPLIANT,
            ...
            assessment_log_entries.append(
                _create_assessment_log_entry(
                    title=f"Analyzed ressource {component_id} is not compliant",
                    description=f"Ressource with id {policy_definition.assignment.id} is not compliant to policy {policy_definition.assignment.id}",
                )
            )
        else:
            findings.append(
                _create_fiding(
                    state=OscalControlComplianceState.COMPLIANT,
                    component_id=component_id,
            ...
            assessment_log_entries.append(
                _create_assessment_log_entry(
                    title=f"Analyzed ressource {component_id} is compliant",
                    description=f"Ressource with id {policy_definition.assignment.id} is compliant to policy {policy_definition.assignment.id}",
                )
            )
...
def _create_fiding(
    state: OscalControlComplianceState, title: str, description: str, component_id: str
) -> OscalCompleteOscalAssessmentCommonFinding:
    return OscalCompleteOscalAssessmentCommonFinding(
        uuid=str(uuid.uuid4()),
        props=[
            OscalCompleteOscalMetadataProperty(
                name=state.value, value=OscalControlComplianceState.NON_COMPLIANT.value
            ),
            OscalCompleteOscalMetadataProperty(
        ...
        target=OscalCompleteOscalAssessmentCommonFindingTarget(
            ...
            status=Status1(state=OscalControlComplianceState.NON_COMPLIANT.value),
...
async def _execute_deploy_tasks(
    assessment_plan: OscalCompleteOscalApAssessmentPlan,
    project: ProjectContext,
    credential: DefaultAzureCredential,
    db: AsyncIOMotorDatabase,
    ...
    """
    Execute the tasks in the assessment plan for the azure plattform -> marked witht CacTaskType.AZURE_DEPLOY_INITIATIVE
    """
    ...
    for deploy_task in deploy_tasks:
        ...
        if policy_definition is None:
            assessment_log_entries.append(
                _create_assessment_log_entry(
                    title="Policy definition not found",
                    description=f"Policy definition with id {policy_initative_id} not found in internal and therefore skipped policy assignment based on search id {search_id}",
                )
            )
            logger.info(
        ...
        if policy_definition.assignment_ids:
            try:
                azure_service.get_assignment_by_id(
                    az_credential=credential,
                    policy_definition=policy_definition,
                    subscription_id=subscription_id,
                )
                ...
                # )
                logger.info(
                    f"Policy with id {policy_initative_id} HAS been assinged. Skipping policy assignment"
                )
                continue
            except HttpResponseError:
                logger.info(
                    f"Policy with id {policy_initative_id} has not been assinged yet"
                )

        assignment = azure_service.create_policy_assignment(
            az_credential=credential,
            azure_path=azure_path,
            policy_definition=policy_definition,
            policy_initative_id=policy_initative_id,
            subscription_id=subscription_id,
        )
        assessment_log_entries.append(
            _create_assessment_log_entry(
                title="Assignt azure policy initiative",
                description=f"Assigned initaitve {assignment.name} to scope {assignment.scope}",
            )
        )
        ...
        await db[MongoDBCollections.POLICY_DEFINITIONS.value].update_one(
            ...
            {
                "$set": {
                    "assignment": AzurePolicyDefinitionAssignment(
...
async def create_policies_and_assessment_plan(
    ...
    _adds_assessment_task_for_components(
        ssp=ssp,
        project=project,
        control_ids=control_ids,
        sub_tasks=sub_tasks,
        policy_set_definitions=policy_set_definitions,
    )

    # Merge azure policies
    await _create_azure_policy_ininiatives(
        policy_set_definitions=policy_set_definitions,
        credential=credential,
        project=project,
        db=db,
...
def _adds_assessment_task_for_components(
    ...
    """
    Creates assessment task for the the individual components in the system-security-plans
    and adds them to sub_tasks. Policy ids are grouped by search path and control_id and
    addedd to polic_set_definitions
    """
...
async def _create_azure_policy_ininiatives(
    policy_set_definitions: list[PolicySetDefinitions],
    credential: DefaultAzureCredential,
    project: ProjectContext,
    db: AsyncIOMotorDatabase,
    ...
    """
    Create azure policy initiative grouped by control id
    """
    ...
    for azure_policy_set in azure_policies_subset:
        ...
        result = azure_service.create_policy(
            az_credential=credential,
            initiative_definition=initiative_definition,
            policy_set_definition_name=policy_set_definition_name,
            subscription_id=policy_client_subscription_id,
        )
